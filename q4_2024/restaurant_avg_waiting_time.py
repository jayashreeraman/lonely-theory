...
customers_2 = [[5,2],[5,4],[10,3],[20,1]]


def return_avg_wait_time(customers):
    num_cust = len(customers)

    wait_times = []

    #wait_times.append(customers[0][1])
    #pit = customers[0][0] + customers[0][1]
    pit = 0
    

    for i in range(len(customers)):
        
        if pit > customers[i][0]:
            pit += customers[i][1]
            #wt = pit - customers[i][0]
        else:
            pit = customers[i][0] + customers[i][1]
            #wt = customers[i][1]
        
        
        #pit = max(pit, customers[i][0]) + customers[i][1]
        wt = pit - customers[i][0]
    ...
    avg_wt_time = sum(wait_times)/num_cust
    return avg_wt_time

print(return_avg_wait_time(customers))

print(return_avg_wait_time([[2,3],[6,3],[7,5],[11,3],[15,2],[18,1]]))
